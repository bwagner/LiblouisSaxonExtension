        ...
        <xsl:choose>
            ...
            <xsl:when test="lang('de')">
                ...
                <xsl:variable name="actual_contraction">
                    <xsl:choose>
                        <xsl:when
                            test="ancestor-or-self::dtb:span[@brl:grade and @brl:grade &lt; $contraction]">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when
                                    test="$detailed_accented_characters = 'de-accents-reduced'">
                                    <xsl:text>sbs-de-accents-reduced.cti,</xsl:text>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:text>sbs-de-accents.cti,</xsl:text>
                ...
                <xsl:if
                    test="$context = 'name_capitalized' or ($context = 'abbr' and not(my:containsDot(.))) or ($actual_contraction = '0' and $context != 'date_day' and $context != 'date_month')">
...
l
        ...
        <xsl:apply-templates select="//dtb:frontmatter/dtb:level1[@class='titlepage']"
            mode="titlepage"/>
        <xsl:text>
b
O
...
y REARe
    ...
    <xsl:template match="brl:num[@role='ordinal' and lang('de')]">
        ...
        <xsl:choose>
        	<xsl:when test="$downshift_ordinals = true()">
                <xsl:value-of
                    select="louis:translate(string($braille_tables), string(translate(.,'.','')))"
                />
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates/>
    ...
    <xsl:template match="brl:num[@role='measure' and lang('de')]">
        ...
        <xsl:variable name="measure" select="(tokenize(string(.), ' '))[position()=last()]"/>
        <xsl:for-each select="tokenize(string(.), ' ')">
    ...
    <xsl:template match="brl:num[@role='isbn' and lang('de')]">
    	...
    	<xsl:variable name="abbr_braille_tables">
    		<xsl:call-template name="getTable">
    			<xsl:with-param name="context" select="'abbr'"></xsl:with-param>
    		</xsl:call-template>   		
    	</xsl:variable>
        ...
        <xsl:choose>
           ...
           letter with abbr -->
            <xsl:when
            	...
            	<xsl:value-of select="louis:translate(string($abbr_braille_tables),concat('&#x2566;',$lastChar))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="clean_number">
                    <xsl:for-each select="tokenize(string(.), ' -')">
